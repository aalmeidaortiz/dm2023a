...
PARAM  <- list()
PARAM$experimento <- "EFE7311_5"

PARAM$exp_input  <- "EDR7211_3"
...
PARAM$CanaritosAsesinos$semilla  <- 100129   # cambiar por la propia semilla
...
if( PARAM$lag1 )
{
  ...
  {
    dataset[ , paste0(vcol, "_delta1") := (get(vcol)  - get(paste0( vcol, "_lag1")))/ get(paste0( vcol, "_lag1")) ]
  }
...
if( PARAM$lag2 )
{
  ...
  for( vcol in cols_lagueables )
  {
    dataset[ , paste0(vcol, "_delta2") := (get(vcol)  - get(paste0( vcol, "_lag2")))/get(paste0( vcol, "_lag2"))  ]
  }
...
if( PARAM$lag3 )
{
  ...
  for( vcol in cols_lagueables )
  {
    dataset[ , paste0(vcol, "_delta3") := (get(vcol)  - get(paste0( vcol, "_lag3")))/get(paste0( vcol, "_lag3"))  ]
  }
