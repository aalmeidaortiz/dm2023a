...
ksemillas  <- c(100019,100069,100103,100109,100129) #reemplazar por las propias semillas
...
setwd("/Users/all-ehoo/Documents/Mineria")  #Establezco el Working Directory
#cargo los datos
...
#OJOOOOOOOO ACAAAAAA.  seagregan las vaiables que faltan
...
#faltaron 2 loops OJOOOO ACAAAAAAA

for( cp  in  c( -0.5, 0, 0.1 )  )
{
for( vmax_depth  in  c( 4, 6, 8, 10, 12, 14, 16 )  )
{
for( vmin_split  in  c( 10, 20, 40, 80, 160, 320, 640 )  )
{
for( minbckt  in  c( 2, 4, 8, 16, 32, vmin_split/4 )  )
  {
  cat("CP: ",cp)
  cat(" MINBCKT: ",minbckt)
  cat(" minsplit: ",vmin_split)
  cat(" maxdepth: ",vmax_depth,"\n")
  #notar como se agrega
  param_basicos  <- list( "cp"=         cp,       #complejidad minima
                          "minsplit"=  vmin_split,  #minima cantidad de registros en un nodo para hacer el split
                          "minbucket"=  minbckt,          #minima cantidad de registros en una hoja
  ...
  #OJOOOOO ACAAAAAAAA para las variables que faltan
  cat(  file=archivo_salida,
        append= TRUE,
        sep= "",
        vmax_depth, "\t",
