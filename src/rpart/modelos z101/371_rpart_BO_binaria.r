...
PARAM$semillas  <- c(100069)        #Mas de una semilla cambia el  1-Repeated
PARAM$crossvalidation  <- 5         #cantidad de folds del cross validation

PARAM$BO$iteraciones  <- 100        #cantidad de iteraciones de la optimizacin bayesiana
...
ArbolSimple  <- function( fold_test, data, param )
{
  ...
  ganancia_testing  <-  dtest[ pred > param2$prob_corte,  sum(ifelse( clase_ternaria=="BAJA+2", 117000, -3000 )) ]
